...
class TestPlanningService:
    ...
    async def test_generate_plan_clones_repository(self) -> None:
        ...
        with patch(
            "troller.worker.services.planning_service.subprocess.run"
        ) as mock_run:
            with patch("troller.worker.services.planning_service.shutil.rmtree"):
                ...
                assert "https://github.com/testowner/testrepo.git" in clone_call[0][0]
    ...
    async def test_generate_plan_cleans_up_repository(self) -> None:
        ...
        with patch(
            ...
            with patch(
                "troller.worker.services.planning_service.shutil.rmtree"
            ) as mock_rmtree:
                mock_run.return_value = MagicMock(returncode=0)
