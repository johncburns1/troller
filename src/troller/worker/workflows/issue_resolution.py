...
class IssueResolutionWorkflow:
    ...
    3. Creates draft pull request
    4. Generates implementation plan
    5. Implements changes via coding agent
    """
    ...
    async def run(
        ...
        # Create draft PR immediately after branch
        pr_body = f"""Resolves #{self._issue.number}

## Description
{self._issue.description}

This PR will implement the changes needed to resolve this issue using autonomous AI agents.
...
"""
        self._pull_request = await workflow.execute_activity(
            ...
            CreatePullRequestInput(
                ...
                base_branch=input.target_branch or "main",
                draft=True,  # Create as draft since planning hasn't happened yet
            ),
            start_to_close_timeout=timedelta(seconds=30),
            retry_policy=RetryPolicy(
                maximum_attempts=3,
